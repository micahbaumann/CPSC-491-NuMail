...
async def mod_chck(reader, writer, message, local_stack, state, loop, action="", what="", params=""):
    if action == "RECEIVE":
        if what == "MAIL":
            ...
            if full_email:
                email_domain = full_email.group(2).strip()
        ...
        if full_email:
            email_domain = full_email.group(2).strip()
            if email_domain == server_settings["visible_domain"] or email_domain == server_settings["domain"] or email_domain == server_settings["public_ip"] or email_domain == server_settings["ip"]:
                # Read confirm not enabled on this server
                writer.write(MessageLine(f"500 6.2.5 {full_email.group(1)}@{email_domain}... Recipient not ok and not receiving read confirmations", message).bytes())
                await writer.drain()
                # mailbox = search_mailbox(full_email.group(1))
                # if mailbox:
                #     if mailbox["mbReadConf"]:
                #         writer.write(MessageLine(f"250 6.2.4 {full_email.group(1)}@{email_domain}... Recipient ok and receiving read confirmations", message).bytes())
                #         await writer.drain()
                #     else:
                #         writer.write(MessageLine(f"500 6.2.5 {full_email.group(1)}@{email_domain}... Recipient not ok and not receiving read confirmations", message).bytes())
                #         await writer.drain()
                ...
                try:
                    ...
                    for domain in loop_range:
                        ...
                        if init:
                            ...
                            if "chck_addr" in DEBUG_VARS.keys():
                                chck_address = DEBUG_VARS["chck_addr"]
                            message_send = await request.send(f"CHCK READ CONFIRM: <{full_email.group(1)}@{chck_address}>")
                            message_split = message_send.split("\r\n")
                            message_split = [i for i in message_split if i]
                            message_list = []
                            for message_part in message_split:
            ...
            writer.write(MessageLine(f"501 Invalid parameterss", message).bytes())
            await writer.drain()
        loop.returnLoop()
    else:
        raise NuMailError(code="7.3.0", message="Module Error")